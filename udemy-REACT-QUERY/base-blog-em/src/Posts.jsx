...
export function Posts() {
  ...
  const { data, isError, error, isLoading } = useQuery(
    ["posts", currentPage], //1.쿼리키 (쿼리 이름)
    () => fetchPosts(currentPage), //2.쿼리 함수 (쿼리에 대한 데이터를 가져오는 비동기 함수) **** fetchPosts의 ${pageNum}에 currentPage 즉 1이 들어가게됨
    {
      //3. 옵션 (2초 뒤에 데이터가 만료되면 그때 refetch)
      staleTime: 2000, // staleTime은 리페칭을 위한 것, 디폴트값은 0임
      keepPreviousData: true,
    }
  );
  console.log("data본문", data);
